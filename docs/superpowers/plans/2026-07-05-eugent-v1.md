...
export const IPC = {
  ...
  dialog: {
    pickDir: 'dialog:pickDir',       // main 端调 dialog.showOpenDialog（第 B1 项）
  },
  events: {
    agent: 'agent:event',
...
- Produces:
  ...
  export interface ChatStreamParams {
    model?: ModelId;                 // 缺省读 settings.selected_model
    messages: OpenAIMessage[];
    tools?: OpenAITool[];
    signal?: AbortSignal;
    thinking?: 'on' | 'off';         // 缺省 'off'；v1 全链路强制关思考（第 3 题 A）
  }
  ...
  export interface OpenAIClientLike {
    stream(params: {
      ...
      tools?: OpenAITool[];
      signal?: AbortSignal;
      thinking: 'on' | 'off';       // Client 层收明确 on/off；ModelManager 已 nullish-coalesce
...
### Task 16: code_exec (worker_threads + vm) + Tools registry
...
**关键设计（第 7 题 A）**：
- `vm.runInContext` 是**同步阻塞**——若在 main 进程直接跑，长脚本会冻结整个 Electron App，chat:cancel 等 IPC 全部堆积。
- v1 改为 `node:worker_threads.Worker` 里跑 `vm.runInNewContext`：worker 独立线程，main 进程不阻塞。
- `signal.aborted` → `worker.terminate()` **立即**返回 `{ok:false, error:'aborted'}`，不等 timeout。
- worker 启动开销 ~30-50ms；code_exec 是 write 类工具，权限弹框本来就要几秒，可接受。
...
try {
  const wrapped = '(function(){return (' + workerData.code + ');})()';
...
export const codeExec: ToolSpec = {
  ...
  run: (args, ctx) =>
    new Promise((resolve) => {
      ...
      let settled = false;
      const settle = (v: { ok: boolean; result?: unknown; error?: string }): void => {
        if (settled) return;
        settled = true;
        ctx.signal.removeEventListener('abort', onAbort);
...
describe('code_exec', () => {
  ...
  it('aborts immediately when signal fires (does not wait for timeout)', async () => {
    ...
    expect(elapsed).toBeLessThan(1_000); // 远小于 10s timeout；证明 worker.terminate() 立即杀
  });
});
```
...
git commit -m "feat(main/tools): code_exec（worker_threads + vm）+ tools registry"
```
  ...
  async select(userInput: string): Promise<SkillSelection> {
    ...
    try {
      ...
      for await (const c of this.model.chatStream({
        model: 'deepseek-v4-flash',
        thinking: 'off',            // 显式关；虽然 ModelManager 默认 'off'，这里显式以防重构时改默认
        messages: [
          {
            role: 'system',
            content:
...
export class ContextBuilder {
  ...
  build(sessionId: string, skill: Skill): { messages: OpenAIMessage[]; tools: OpenAITool[] } {
    // 过滤中断产生的半截 assistant 消息（partial=true）——避免半截文本带偏后续回答（第 4 题 A）
...
describe('ContextBuilder', () => {
  ...
  it('excludes partial assistant messages from context (第 4 题 A)', () => {
    const { memory, cb } = make();
    const s = memory.createSession(1000);
...
### Task 21: IPC router + sessions/settings/dialog handler

**Files:**
- Create: `packages/main/src/ipc/router.ts`
- Create: `packages/main/src/ipc/sessions.ts`
- Create: `packages/main/src/ipc/settings.ts`
- Create: `packages/main/src/ipc/dialog.ts`   (第 B1 项：workspace picker 用 `dialog.showOpenDialog`)
...
export function registerIpc(services: AppServices, getWindow: () => BrowserWindow | null): void {
  ...
  registerDialog(getWindow);          // 第 B1 项：workspace picker
}
```

- [ ] **Step 3.5: packages/main/src/ipc/dialog.ts**
...
// （替代 Task 30 里 window.prompt() —— sandbox renderer 里 prompt 不可用）
export function registerDialog(getWindow: () => BrowserWindow | null): void {
  ipcMain.handle(
    IPC.dialog.pickDir,
    async (): Promise<{ ok: true; path: string } | { ok: false; error: 'cancelled' }> => {
      const win = getWindow();
...
**注意**：router.ts 顶部需要 `import { registerDialog } from './dialog.js';`

- [ ] **Step 4: commit**

```bash
git add packages/main/src/ipc/router.ts packages/main/src/ipc/sessions.ts packages/main/src/ipc/settings.ts packages/main/src/ipc/dialog.ts
git commit -m "feat(main/ipc): sessions + settings + dialog + router 骨架"
```
...
- Produces: 挂到 `window.eugent` 的强类型 API：
  ...
  export interface EugentApi {
    ...
    dialog: {
      pickDir(): Promise<{ ok: true; path: string } | { ok: false; error: 'cancelled' }>;
    };
...
export function ApprovalDialog() {
  ...
  async function pickDir() {
    // 第 B1 项：走 main 侧 dialog.showOpenDialog（sandbox renderer 里 window.prompt 不可用）
...
> Note: 早期 plan 用过 `window.prompt()` 但 Electron sandbox renderer 里不可用；grill 后已改为 main 侧 `dialog.showOpenDialog`（Task 21 registerDialog + Task 24 preload 暴露）。
...
git commit -m "feat(renderer): ApprovalDialog + workspace 懒选（走 main 侧 dialog.showOpenDialog）"
```
...
- [ ] **Step 4: 扩展 fake-deepseek.ts 支持 tool_calls 场景**

在 `tests/e2e/fake-deepseek.ts` 里追加一个变体：
...
- [ ] **Step 5: tests/e2e/tool-approval.spec.ts**
...
test('触发文件类工具 → 弹权限 → 允许 → 结果回传 → 助手完成回复', async () => {
  ...
  // 预设 workspace_dir，跳过 pick_workspace 交互（v1 用 main 侧 dialog.showOpenDialog，
  // E2E 里直接注入 settings 更简单）
  const ws = mkdtempSync(join(tmpdir(), 'eugent-e2e-ws-'));
  await page.evaluate(
    (dir) => window.eugent.settings.set({ workspaceDir: dir }),
    ws,
  );
  ...
  // file_list 是 read 类工具，v1 不弹权限（自动放行）。若换成 file_write / shell，
  // 这里会看到"允许调用工具..."弹框。
  // 为验证 approval 路径，用 shell 版脚本 —— 这里保留 file_list 场景验证工具卡片 + 结果。
...
> 注：本 E2E 用 read 类 `file_list` 走"工具调用→结果回传"主路径。write 类工具的权限弹框在单元测试（Task 11 ToolManager）与集成测试（Task 20 AgentLoop）里覆盖；如需 E2E 覆盖 approval UI，把 fake 里 `file_list` 换成 `shell`，脚本中加"点仅本次允许"步骤即可（不再重复贴）。
...
git commit -m "test(e2e): 聊天流式 + 工具调用回传（fake DeepSeek endpoint）"
```
